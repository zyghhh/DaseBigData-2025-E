...
class ExperimentAnalyzer:
    ...
    def plot_internal_fault_latency_comparison(self):
        ...
        ax1.bar(x - width/2, flink_sorted['p95_latency'].values, width, 
               label='Flink', color=FLINK_COLOR, alpha=0.8)
        ax1.bar(x + width/2, storm_sorted['p95_latency'].values, width, 
               label='Storm', color=STORM_COLOR, alpha=0.8)
        
        ax1.set_xlabel('Fault Frequency', fontsize=11, fontweight='bold')
        ax1.set_ylabel('P95 Latency (ms)', fontsize=11, fontweight='bold')
        ax1.set_title('P95 Latency Comparison', fontsize=12, fontweight='bold')
        ax1.set_xticks(x)
        ax1.set_xticklabels([f"{int(r['total_count'])}" for _, r in flink_sorted.iterrows()])
        ax1.legend(fontsize=10)
        ax1.grid(axis='y', alpha=0.3)
        
        # P99延迟
        ax2.bar(x - width/2, flink_sorted['p99_latency'].values, width, 
               label='Flink', color=FLINK_COLOR, alpha=0.8)
        ax2.bar(x + width/2, storm_sorted['p99_latency'].values, width, 
               label='Storm', color=STORM_COLOR, alpha=0.8)
        
        ax2.set_xlabel('Fault Frequency', fontsize=11, fontweight='bold')
        ax2.set_ylabel('P99 Latency (ms)', fontsize=11, fontweight='bold')
        ax2.set_title('P99 Latency Comparison', fontsize=12, fontweight='bold')
        ax2.set_xticks(x)
        ax2.set_xticklabels([f"{int(r['total_count'])}" for _, r in flink_sorted.iterrows()])
        ax2.legend(fontsize=10)
        ax2.grid(axis='y', alpha=0.3)
        
        plt.suptitle('Internal Fault: Latency Impact\n(Higher fault count = higher frequency, every N messages inject 1 fault)', 
    ...
    def plot_external_fault_p95_latency_by_count(self):
        """图表9: 外部故障P95延迟 vs 故障次数（折线图）"""
        if not self.external_fault_data:
            return
        print("\n📈 生成图表9: 外部故障P95延迟 vs 故障次数...")
        self._plot_metric_by_count('p95_latency', 'P95 Latency (ms)', 
    ...
    def plot_external_fault_p99_latency_by_count(self):
        """图衐10: 外部故障P99延迟 vs 故障次数（折线图）"""
        if not self.external_fault_data:
            return
        print("\n📈 生成图衐10: 外部故障P99延迟 vs 故障次数...")
        self._plot_metric_by_count('p99_latency', 'P99 Latency (ms)', 
    ...
    def plot_external_fault_avg_latency_by_count(self):
        """图衐11: 外部故障平均延迟 vs 故障次数（折线图）"""
        if not self.external_fault_data:
            return
        print("\n📈 生成图衐11: 外部故障平均延迟 vs 故障次数...")
        self._plot_metric_by_count('avg_latency', 'Avg Latency (ms)', 
    ...
    def generate_summary_report(self):
        ...
        if self.baseline_data is not None:
            ...
            for _, row in self.baseline_data.iterrows():
                ...
                report.append(f"  - P50 延迟: {row['p50_latency']:.2f} ms")
