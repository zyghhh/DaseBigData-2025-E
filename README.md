...
| 框架 | P95 延迟 (ms) | P99 延迟 (ms) | 吞吐量 (msg/s) |
|---------|-------------|-------------|----------------|
| **Flink** | 409,799 | 424,702 | 573.93 |
| **Storm** | 24,376 | 25,332 | 3,521.42 |

**故障参数：总次数=150，消息间隔=2000（每 2000 条消息注入一次）**

| 框架 | P95 延迟 (ms) | P99 延迟 (ms) | 吞吐量 (msg/s) |
|---------|-------------|-------------|----------------|
| **Flink** | 316,481 | 320,549 | 721.81 |
| **Storm** | 21,363 | 22,149 | 3,514.12 |

**故障参数：总次数=200，消息间隔=1500（每 1500 条消息注入一次）**

| 框架 | P95 延迟 (ms) | P99 延迟 (ms) | 吞吐量 (msg/s) |
|---------|-------------|-------------|----------------|
| **Flink** | 603,993 | 641,089 | 402.95 |
| **Storm** | 30,333 | 31,357 | 3,487.12 |

**分析**：

- **Flink 延迟与吞吐量双重崩溃**：
  - P99 延迟从基线的 4.3 秒暴涨至 **10 分钟以上**（641,089ms），吞吐量从 3,040 msg/s 暴跌至 **403 msg/s**；
  - Task 重启导致数据积压，恢复后需“追赶”进度，Checkpoint 回滚引发大量重复处理，严重占用处理资源。

- **Storm 延迟与吞吐量保持稳定**：
  - P99 延迟始终保持在 **25–31 秒**范围内，吞吐量稳定在 **3,487–3,521 msg/s**；
...
| 故障次数 | 框架 | P99 延迟 (ms) | 吞吐量 (msg/s) |
|----------|---------|-------------|----------------|
| 1 | **Flink** | 17,435 | 3,029.65 |
| 1 | **Storm** | 23,802 | 3,057.31 |
| 2 | **Flink** | 21,982 | 3,015.54 |
| 2 | **Storm** | 75,312 | 3,048.83 |
| 3 | **Flink** | 27,731 | 3,095.53 |
| 3 | **Storm** | 67,560 | 3,086.48 |
| 4 | **Flink** | 35,669 | 3,047.12 |
| 4 | **Storm** | 88,989 | 3,051.28 |

**分析**：

- **延迟对比**：Flink 延迟随故障次数缓慢上升（17–36 秒），Storm 延迟急剧恶化（24–89 秒）；
...
| 故障次数 | 框架 | P99 延迟 (ms) | 吞吐量 (msg/s) |
|----------|---------|-------------|----------------|
| 1 | **Flink** | 121,788 | 3,752.35 |
| 1 | **Storm** | 14,691 | 3,078.97 |
| 2 | **Flink** | 128,679 | 3,683.87 |
| 2 | **Storm** | 16,743 | 3,088.89 |
| 3 | **Flink** | 122,903 | 3,753.00 |
| 3 | **Storm** | 11,237 | 3,060.46 |
| 4 | **Flink** | 128,947 | 3,690.00 |
| 4 | **Storm** | 15,289 | 3,081.37 |

**分析**：

- **延迟对比**：Flink P99 延迟高达 **122–129 秒**，Storm 稳定在 **11–17 秒**；
- **吞吐量对比**：Flink 吞吐量达到 **3,683–3,753 msg/s**（高于基线的 3,040 msg/s），这是因为大量重复处理提高了"处理总量"但并非有效吞吐；Storm 吞吐量稳定在 **3,060–3,089 msg/s**；
- **原因**：Flink 的 JobManager 是全局状态协调者，故障需要整体恢复；Storm 的 Nimbus 仅负责调度，Worker 自治性强。
...
| 故障次数 | 框架 | P99 延迟 (ms) | 吞吐量 (msg/s) |
|----------|---------|-------------|----------------|
| 1 | **Flink** | 91,752 | 3,019.39 |
| 1 | **Storm** | 11,859 | 3,061.89 |
| 2 | **Flink** | 167,701 | 2,649.13 |
| 2 | **Storm** | 12,851 | 3,059.65 |
| 3 | **Flink** | 275,073 | 2,314.76 |
| 3 | **Storm** | 15,852 | 3,056.01 |
| 4 | **Flink** | 359,270 | 2,084.53 |
| 4 | **Storm** | 15,719 | 3,079.83 |

**分析**：

- **延迟对比**：Flink P99 延迟从 91,752ms 暴涨至 **359,270ms（近 6 分钟）**，Storm 始终稳定在 **12–16 秒**；
- **吞吐量对比**：Flink 吞吐量从 3,019 msg/s **暴跌至 2,085 msg/s（下降 31%）**，Storm 始终稳定在约 **3,060 msg/s**；
- **原因**：网络隔离导致 Flink TaskManager 心跳超时，触发全局 Checkpoint 回滚，隔离时间越长积压越严重；Storm 的 Tuple Tree 机制允许单个 Worker 暂时失联后快速重连，仅重发少量未 Ack 消息。
